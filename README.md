...
Sau khi chạy node, mở trình duyệt và truy cập:
...
### Consistent Hashing
Key được ánh xạ tới node bằng hàm **MD5 Hash**.
...
### Heartbeat
Các node gửi request:

```
/heartbeat
```

để kiểm tra node còn hoạt động.

### Data Recovery
Khi node khởi động lại, node sẽ gọi:

```
/snapshot_for
```

để lấy lại dữ liệu cần thiết từ các node khác.
